...
\*\*Supersedes: DEV\_DesignArchitecture\_10.2\_20260120 and all previous versions and addendums\*\*
...
5\. System Architecture    
   ...
   5.4 Frontend-Backend Runtime Integration    
...
6\. Key Service Logic    
   ...
   6.3 BudgetService    
   6.4 Days Remaining Semantics — Zero-Line Cash Exhaustion

7\. Reconciliation System    
   7.1 30-Day Baseline    
   7.2 Rolling Anchor Model (Free Tier)
...
18\. File Structure Summary    
    18.2 Repository Separation Model

19\. Storage Specification (Free Tier)    
    19.1 Core Principles    
    19.2 Storage Domains    
    19.3 Backup & Restore    
    19.4 Storage Guardrail    
    19.5 Import, Export, and Auto-Export Operational Semantics

20\. Ad Placement Specification (Free Tier)    
...
21\. Frontend Styling Architecture — Token-Only, Component-Scoped Styling    
    21.1 Core Principle (MUST)    
    21.2 Layers & Responsibilities    
    21.3 Semantics & Logic vs Styling    
    21.4 Button Semantics, Token Usage, and CSS Cleanup Protocol    
    21.5 AI Execution Rules (Non-Negotiable)    
    21.6 Icon Infrastructure Canonicalization & Token Correction

22\. Transaction Lifecycle & Frontend Workflow Semantics    
...
23\. Projection Correctness & Planned Transaction Semantics    
...
29\. Essential Tier Projection Stability & Balance Anchoring    
    29.1 Status: DEFERRED (Mandatory before Essential Release)    
    29.2 Requirements (Deferred)

30\. Versioning

31\. Design Tokens System & Domain Strategy    
    31.1 Purpose and Scope    
    31.2 Token System Reference    
...
32\. AI Discovery & Semantic Integrity Constraints    
...
33\. Frontend Layout System & View Architecture    
...
34\. Frontend Implementation Status & Patterns    
...
The documentation system is centralized in a dedicated repository and automatically mirrored into each code repository. This eliminates multi-repo doc drift while keeping implementation simple and low cognitive:

\- \*\*Single source of truth\*\*: \`fortunetell-docs\`  
\- \*\*Mirrored destinations\*\* (read-only, automation-owned docs):  
  \- \`fortunetell-backend\`  
  \- \`fortunetell-frontend\`  
  \- \`fortunetell-site\`  
  \- \`simple-budget-site\`  
\- \*\*Sync mechanism\*\*: GitHub Actions workflow in \`fortunetell-docs\`  
\- \*\*Guardrails\*\*: GitHub Actions workflows in each code repo that block direct edits to \`/docs\`

\---

\#\#\#\# \*\*10.3.2 Ownership Model\*\*

\*\*Authoritative editing location\*\*

\- All human-authored documentation changes are made only in:  
  \- \`fortunetell-docs/docs\`

\*\*Mirrored copies\*\*

\- Each code repo contains a \`docs/\` directory at the repo root.  
\- These \`docs/\` directories are treated as automation-owned mirrors of \`fortunetell-docs/docs\`.  
\- Any manual edits in code-repo \`/docs\` are considered invalid and are prevented at CI time.

\*\*Summary of ownership rules\*\*

\- \*\*Allowed\*\*: Editing documentation in \`fortunetell-docs/docs\` on the \`main\` branch.  
\- \*\*Not allowed\*\*: Editing \`docs/\*\*\` directly in:  
  \- \`fortunetell-backend\`  
  \- \`fortunetell-frontend\`  
  \- \`fortunetell-site\`  
  \- \`simple-budget-site\`
...
\- \*\*Central docs repo\*\*  
  \- Repo: \`fortunetell-docs\`  
  \- Docs root: \`/docs\`

\- \*\*Code repos\*\*  
  \- Repo: \`fortunetell-backend\`  
    \- Docs mirror: \`/docs\`  
  \- Repo: \`fortunetell-frontend\`  
    \- Docs mirror: \`/docs\`  
  \- Repo: \`fortunetell-site\`  
    \- Docs mirror: \`/docs\`  
  \- Repo: \`simple-budget-site\`  
    \- Docs mirror: \`/docs\`

No additional hierarchy or "semantic" structure is imposed beyond the flat \`/docs\` subtree. The sync job copies files byte-for-byte.

\---

\#\#\#\# \*\*10.3.4 Sync Workflow (GitHub Actions, \`fortunetell-docs\`)\*\*

A single GitHub Actions workflow in \`fortunetell-docs\` is responsible for distributing docs to all code repos.
...
Any push to \`main\` in \`fortunetell-docs\` that modifies \`docs/\*\*\` triggers a sync run.

\*\*Authentication\*\*

\* Uses a fine-grained GitHub PAT with:

  \* Resource: the four code repos  
  \* Permission: \`contents: read/write\`  
\* Token is stored as a repository secret in \`fortunetell-docs\`:

  \* \`DOCS\_SYNC\_TOKEN\`  
\* The workflow uses \`github.repository\_owner\` to avoid hardcoding the org/user.

\*\*Sync behavior (per run)\*\*

1\. Check out \`fortunetell-docs\` into a subdirectory (e.g. \`docs-source\`).  
2\. Compute the short SHA of the triggering commit:

   \* \`short\_sha \= \<first 7 chars of GITHUB\_SHA\>\`  
3\. For each target repo, sequentially (no matrices/loops):

   \* Check out the target repo into its own directory using \`actions/checkout@v4\` with:

     \* \`repository: ${{ github.repository\_owner }}/\<target-repo-name\>\`  
     \* \`token: ${{ secrets.DOCS\_SYNC\_TOKEN }}\`  
     \* \`path: \<local-dir\>\`  
   \* Replace the \`docs/\` directory:

     \* \`rm \-rf docs\`  
     \* \`mkdir \-p docs\`  
     \* \`cp \-R ../docs-source/docs/. docs/\`  
   \* Configure a generic bot identity:

     \* \`git config user.name "docs-sync-bot"\`  
     \* \`git config user.email "docs-sync-bot@users.noreply.github.com"\`  
   \* Stage \`docs\` and detect change:

     \* If \`git diff \--cached \--quiet\`: no changes → skip commit/push for that repo.  
     \* Otherwise:

       \* Commit with:

         \* \`chore(docs): sync from fortunetell-docs@\<short\_sha\>\`  
       \* Push to \`main\`.

\*\*Notes\*\*

\* Each target repo update is independent; failure on one repo does not implicitly modify the others.  
\* If \`docs/\` is unchanged relative to the last sync for a specific repo, that repo's section of the job is a no-op (no commit).

\---

\#\#\#\# \*\*10.3.5 Guardrail Workflow (Code Repos)\*\*

Each code repo includes a lightweight CI workflow that prevents human edits to \`/docs\`.

\*\*Workflow location (per repo)\*\*

\* \`fortunetell-backend/.github/workflows/docs-guardrail.yml\`  
\* \`fortunetell-frontend/.github/workflows/docs-guardrail.yml\`  
\* \`fortunetell-site/.github/workflows/docs-guardrail.yml\`  
\* \`simple-budget-site/.github/workflows/docs-guardrail.yml\`
...
The workflow runs on any PR that touches \`docs/\*\*\`, regardless of default branch name.

\*\*Behavior\*\*

\* Checks out the repo with full history (\`fetch-depth: 0\`).  
\* Computes the base and head SHAs for the PR.  
\* Lists changed files between base and head.  
\* Filters changes for paths under \`docs/\`.

If any \`docs/\` paths are present:

\* The job emits an error message similar to:

  \> Changes to /docs are not allowed in this repo.  
  \> Docs under /docs are automation-owned and synced from the central fortunetell-docs repo.  
  \> Edit docs only in the fortunetell-docs repo and let the sync workflow propagate changes.

\* The job exits with non-zero status → PR CI fails.

If no \`docs/\` paths are present:

\* The job prints "No /docs changes detected in this PR. Guardrail passing."  
\* The job exits successfully.

\*\*Effect\*\*

\* Any attempt to edit code-repo \`/docs\` via PR is blocked by CI.  
\* Automation-initiated direct commits to \`main\` (from \`docs-sync.yml\`) do not involve PRs and therefore bypass this guardrail, as intended.
...
\* If \`DOCS\_SYNC\_TOKEN\` is missing, revoked, or lacks \`contents: read/write\`:

  \* The docs-sync workflow will fail when attempting to push to target repos.  
  \* Resolution:

    \* Regenerate a fine-grained PAT with correct repo/permission scope.  
    \* Update the \`DOCS\_SYNC\_TOKEN\` secret in \`fortunetell-docs\`.

\*\*Branch protection\*\*

\* If a target repo enforces branch protection rules that block direct pushes to \`main\` for the bot identity:

  \* The push step in the docs-sync job will fail for that repo.  
  \* Options:

    \* Adjust branch protection to allow the docs-sync bot to push.  
    \* Or redesign that repo's sync mode to use PR-based updates instead of direct commits (not currently implemented in this Option B configuration).

\*\*No-op sync\*\*

\* If the content of \`docs/\` in \`fortunetell-docs\` has not changed relative to what's already in a target repo:

  \* The workflow performs a copy, but \`git diff \--cached \--quiet\` detects no changes.  
  \* No commit or push is created for that repo; this is normal and expected.

\---

\#\#\#\# \*\*10.3.7 Scope and Non-Goals\*\*

\* No changes to application behavior, build behavior, or architecture.  
\* No semantic transformation of documentation; the workflow performs a straight file copy.  
\* No monorepo or directory refactors tied to this system.  
\* No external paid services; only GitHub-hosted runners and Actions.

The docs sync \+ guardrail system is purely a distribution and governance mechanism to keep a single documentation source of truth (\`fortunetell-docs\`) mirrored into multiple code repositories without manual coordination.
